...
性能：2547 个时间步彼此独立（sweep 事件已预计算成 equip_events[eid][t]，无跨步状态），
    ...
    with open(chunk_csv_path, mode='w', newline='', encoding='utf-8') as f:
        ...
        for t in range(t_start, t_end + 1):
            ...
            for radar_id in radar_keys:
                ...
                if alive_targets:
                    # 向量化：一次算出本雷达对全部存活目标的 az/el/斜距，再统一判可视
                    az, el, srange = pm.geodetic2aer(
                        alive_lats, alive_lons, alive_alts,
                        radar_info.latitude, radar_info.longitude, radar_info.altitude,
                    )
                    az = np.asarray(az); el = np.asarray(el); srange = np.asarray(srange)
                    rng_km = srange / 1000.0
                    detect = (
                        (radar_info.azi_min <= az) & (az <= radar_info.azi_max) &
                        (radar_info.ele_min <= el) & (el <= radar_info.ele_max) &
                        (radar_info.range_min <= rng_km) & (rng_km <= radar_info.range_max)
                    )
            ...
            for sat_id in sat_keys:
                ...
                if alive_targets:
                    # 向量化：卫星位置只算一次，全部目标一次 geodetic2ecef + 视场夹角判定
                    sat_xyz = np.asarray(pm.geodetic2ecef(s.latitude, s.longitude, s.altitude))
                    tgt_x, tgt_y, tgt_z = pm.geodetic2ecef(alive_lats, alive_lons, alive_alts)
                    targets_xyz = np.column_stack((np.asarray(tgt_x), np.asarray(tgt_y), np.asarray(tgt_z)))  # (N, 3)
