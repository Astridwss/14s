...
class SituationJSONLogger:
    ...
    @staticmethod
    def _extract_equips_and_actions(obs, actions, agent_keys, satellite_keys, target_keys, action_masks):
        """抽取装备状态、掩码可见目标与实际指令（雷达/卫星统一，按 type 区分）。

        多标签雷达一步会发多条锁定指令，按装备 id 归并成列表展示。
        """
        # 装备 id → 锁定目标列表（逐条归并，避免旧实现 dict 覆盖丢目标）
        action_map = {}
        for cmd in actions:
            eq_id = getattr(cmd, 'str_equip_id', '')
            tgt = getattr(cmd, 'str_target_id', '')
            if not eq_id or not tgt:
                continue
            action_map.setdefault(eq_id, []).append(tgt)

        sat_set = set(satellite_keys or [])

        equip_list = []
        for i, eq_id in enumerate(agent_keys):
            equip_state = obs.dict_equip_state.get(eq_id)
            if not equip_state:
                continue
            ...
            locked_targets = action_map.get(eq_id, [])
            action_str = ",".join(locked_targets) if locked_targets else "STANDBY (待机)"

            equip_list.append({
                "sensor_id": eq_id,
                "type": "satellite" if eq_id in sat_set else "radar",
                "load": load_str,
    ...
    def generate_step_json(cls, step: int, obs, actions,
                           agent_keys, satellite_keys, target_keys, action_masks, reward: float) -> str:
        """拼装结构化 JSON 字符串。"""
        if not obs:
            return "{}"
        log_dict = {
            ...
            "equips": cls._extract_equips_and_actions(obs, actions, agent_keys, satellite_keys, target_keys, action_masks),
    ...
    def print_log(cls, step: int, obs, actions,
                  agent_keys, satellite_keys, target_keys, action_masks, reward: float):
        """控制台打印入口。"""
        if not obs:
            return

        equips_data = cls._extract_equips_and_actions(obs, actions, agent_keys, satellite_keys, target_keys, action_masks)
        targets_data = cls._extract_target_status(obs)

        print(f"\n[{'=' * 15} 态势日志 Step: {step} | "
              f"Time: {obs.current_time}s | Reward: {reward:.2f} {'=' * 15}]")

        print(f"装备状态 ({len(equips_data)} 部):")
...
class SituationLogHook:
    ...
    用法:
        log_hook = SituationLogHook(agent_keys, target_keys, satellite_keys, log_interval=50)
        worker.generate_train_episode(epsilon=0.1, step_hooks=[log_hook])
    """

    def __init__(self, agent_keys: List[str], target_keys: List[str],
                 satellite_keys: Optional[List[str]] = None,
                 log_interval: int = 50):
        self.agent_keys = agent_keys
        self.target_keys = target_keys
        self.satellite_keys = satellite_keys or []
    ...
    def __call__(self, step_count: int, terminated: bool, truncated: bool,
        ...
        SituationJSONLogger.print_log(
            ...
            satellite_keys=self.satellite_keys,
